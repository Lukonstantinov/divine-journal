...
### Version Management
- **Current version: v5.8 (native/Expo)**, **v5.11 (web)** — the two apps have drifted; see "Enhancement Plan" below for what's web-only vs native-only.
...
- **v5.11 (web only) — UI redesign pass**: Introduced a shared design-system layer in `web/src/index.css` rather than a per-screen rewrite: `data-theme` attribute set on `<html>` in `App.tsx` so CSS can react to the active theme without threading it through every stylesheet rule; `.sheet-panel` class (drag handle via `::before`, elevation shadow, slide-up entrance animation) applied to every bottom sheet across all 9 files that have one (BibleScreen, CalendarScreen, DailyReadingModal, FolderManager, GraphView, JournalScreen, SearchScreen, VerseFormatModal, VersePicker); `.modal-backdrop` (blur + fade-in) made consistent on every overlay, including the ones that were missing it; `.card-elevated` (subtle shadow + press-scale) applied to journal entry cards; `.press` (tactile scale-down on tap) applied app-wide to every interactive button alongside the existing `active:opacity-70`. `TabBar.tsx` fully rewritten as a floating rounded pill nav with an animated active-tab background and icon "pop" on switch. Not done in this pass: native app parity, a literal FAB-per-screen, and per-component bespoke layouts — see "Planned" below and the architecture notes in chat for the fuller redesign roadmap (native/web unification, new features like verse cross-refs, tags, sharing, cloud sync).
